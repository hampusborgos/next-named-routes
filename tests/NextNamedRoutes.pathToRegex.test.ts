...
describe('NextNamedRoutes.pathToRegex', () => {
  it('translates to correct regex', () => {
    ...
    expect(
      NextNamedRoutes.pathToRegex('/[organizationSlug]/tema/[topicSlug]')
        .source,
    ).toEqual('^\\/(?:([^/]+?))\\/tema\\/(?:([^/]+?))$')
  })
  ...
  it('regex matches not matching shorter paths', () => {
    expect(
      NextNamedRoutes.pathToRegex('/[organizationSlug]').test('/'),
    ).toBeFalsy()
  })
  ...
  it('regex matches paths', () => {
    ...
    expect(
      NextNamedRoutes.pathToRegex('/[organizationSlug]').test('/test-as'),
    ).toBeTruthy()
